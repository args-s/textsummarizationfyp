...
#    __doc__ += "Notes\n%s-----" % (indent,) + sources.replace('\n','\n'+indent)
            ...
            for node_1 in intersecn_T1:
                for node_2 in intersecn_T2:
                    if (node_1 == node_2) or (head_word(node1) == head_word(node2)):
                        ...
                        z = Map2Graphs.wn_sim(node_1, node_2, 'n')
                        if z < best_score:
                            best_1 = node_1
                            best_2 = node_2
                            best_score = z
        ...
        if intersecn_T1 and intersecn_T2:
            for node_1 in intersecn_T1:
                for node_2 in intersecn_T2:
                    ...
                    if node_1.split(separator)[0] == node_2.split(separator)[0]:   # node identicality
                        yield node_1, node_2
        ## end dod2020
        T1_out = self.remove_mapped(T1_out)
        T2_out = self.remove_mapped2(T2_out)
    ...
    def semantic_feasibility(self, G1_node, G2_node):
        if G1_node == G2_node:    #dod
            ...
            if head1 == head2:
                return True
            elif Map2Graphs.wn_sim(w1, w2, 'n'):
                return True
            else:
                return False 
...
def head_word(G_node):
    if separator in G_node:     # separator = ':' or '_'
        split1 = G_node.split(separator)[0]  # C# Java nodes
        return split1
    else:
        return G_node
